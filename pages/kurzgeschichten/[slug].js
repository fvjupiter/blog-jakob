...
export default function Story({
  ...
  const cNButtons = `text-3xl lg:text-5xl text-blue-900 hover:text-blue-600 dark:text-blue-100 dark:hover:text-blue-400 duration`;
  ...
  const PrevStory = () => {
    ...
    if (prevStory) {
      return (
        <Link href={`/kurzgeschichten/${prevStory.fields.slug}`}>
          <a className={`${cNButtons} left-0`}>
            <BsChevronLeft />
          </a>
        </Link>
      );
      ...
      return (
        <Link
          href={`/kurzgeschichten/${stories[stories.length - 1].fields.slug}`}
        >
          <a className={`${cNButtons} left-0`}>
            <BsChevronLeft />
          </a>
  ...
  const NextStory = () => {
    ...
    if (nextStory) {
      return (
        <Link href={`/kurzgeschichten/${nextStory.fields.slug}`}>
          <a className={`${cNButtons} right-0`}>
            <BsChevronRight />
          </a>
        </Link>
      );
      ...
      return (
        <Link href={`/kurzgeschichten/${stories[0].fields.slug}`}>
          <a className={`${cNButtons} right-0`}>
            <BsChevronRight />
          </a>
        </Link>
    ...
    return (
      <div className="mb-40 px-4 lg:px-24">
        ...
        <div
          ...
          from-white to-blue-50 dark:bg-gradient-to-br dark:from-stone-800 dark:to-stone-900
          border-blue-400 ring-blue-900 text-blue-900 dark:text-blue-100
